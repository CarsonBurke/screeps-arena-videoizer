...
impl Nv12ReadbackLayout {
    ...
    fn unpack(self, mapped: &[u8], active_layers: u32) -> Result<Vec<Vec<u8>>> {
        ...
        for layer in 0..active_layers {
            ...
            let mut frame = Vec::with_capacity(self.tight_frame_bytes);
            for row in 0..self.height {
                let start = frame_offset
                    .checked_add(row as usize * self.row_stride as usize)
                    .ok_or(Error::ArithmeticOverflow)?;
                frame.extend_from_slice(&mapped[start..start + self.width as usize]);
            }
...
impl Nv12ReadbackBuffer {
    ...
    fn read_submitted(
        ...
        let frames = self.layout.unpack(&mapped, active_layers);
        drop(mapped);
        self.buffer.unmap();
        frames
    }
...
pub fn validate_nv12_shader() -> Result<()> {
    let module = naga::front::wgsl::parse_str(NV12_SHADER)
        .map_err(|error| Error::Invalid(format!("NV12 conversion WGSL is invalid: {error}")))?;
    naga::valid::Validator::new(
        naga::valid::ValidationFlags::all(),
        naga::valid::Capabilities::all(),
    )
    .validate(&module)
    .map_err(|error| Error::Invalid(format!("NV12 conversion WGSL is unsupported: {error:#?}")))?;
    Ok(())
}
