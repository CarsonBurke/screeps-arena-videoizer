...
impl Session {
    ...
    /// Register an external resource once while mapping it only around each
    /// encode operation.
    ///
    /// This is appropriate for resources shared with another graphics API:
    /// the registration may remain resident, but callers must drop every
    /// [`MappedRegisteredResource`] before the other API accesses the resource.
    ///
    /// # Errors
    ///
    /// Could error if registration fails, the resource is invalid, or the
    /// encoder runs out of memory.
    pub fn register_persistent_generic_resource<T>(
        &self,
        marker: T,
        resource_type: NV_ENC_INPUT_RESOURCE_TYPE,
        resource_to_register: *mut c_void,
        pitch: u32,
    ) -> Result<Rc<PersistentRegisteredResource<'_, T>>, EncodeError> {
        let mut parameters = NV_ENC_REGISTER_RESOURCE::new(
            resource_type,
            self.width,
            self.height,
            resource_to_register,
        ...
        .pitch(pitch);
        unsafe { (ENCODE_API.register_resource)(self.encoder.ptr, &mut parameters) }
            .result(&self.encoder)?;
        Ok(Rc::new(PersistentRegisteredResource {
            reg_ptr: parameters.registeredResource,
            pitch,
            encoder: &self.encoder,
...
/// A long-lived external-resource registration that can be mapped repeatedly.
///
/// The marker keeps the external allocation alive until NVENC unregisters it.
/// Use [`Self::map`] to create the short-lived encoder input for one use.
#[derive(Debug)]
pub struct PersistentRegisteredResource<'a, T> {
    reg_ptr: *mut c_void,
    pitch: u32,
    encoder: &'a Encoder,
    mapped: Cell<bool>,
...
impl<'a, T> PersistentRegisteredResource<'a, T> {
    /// Map this registration for an encode operation.
    ///
    /// # Errors
    ///
    /// Returns an NVENC error if the mapping operation fails.
    ///
    /// # Panics
    ///
    /// Panics if this resource already has a live mapped guard.
    pub fn map(self: &Rc<Self>) -> Result<MappedRegisteredResource<'a, T>, EncodeError> {
        assert!(
            !self.mapped.replace(true),
            "an NVENC resource cannot be mapped more than once"
        );
        let mut parameters = NV_ENC_MAP_INPUT_RESOURCE {
            version: NV_ENC_MAP_INPUT_RESOURCE_VER,
            registeredResource: self.reg_ptr,
            mappedResource: ptr::null_mut(),
            mappedBufferFmt: NV_ENC_BUFFER_FORMAT::NV_ENC_BUFFER_FORMAT_UNDEFINED,
            ..Default::default()
        };
        if let Err(error) =
            unsafe { (ENCODE_API.map_input_resource)(self.encoder.ptr, &mut parameters) }
                .result(self.encoder)
        {
            self.mapped.set(false);
            return Err(error);
        }
        Ok(MappedRegisteredResource {
            resource: Rc::clone(self),
            map_ptr: parameters.mappedResource,
        })
    }
}

impl<T> Drop for PersistentRegisteredResource<'_, T> {
    fn drop(&mut self) {
        debug_assert!(!self.mapped.get());
        unsafe { (ENCODE_API.unregister_resource)(self.encoder.ptr, self.reg_ptr) }
            .result(self.encoder)
            .expect("The encoder pointer and registered handle should be valid.");
    }
}

/// A mapped view of a [`PersistentRegisteredResource`].
///
/// Dropping this guard unmaps the input while preserving its registration for
/// a later encode operation.
#[derive(Debug)]
pub struct MappedRegisteredResource<'a, T> {
    resource: Rc<PersistentRegisteredResource<'a, T>>,
    map_ptr: *mut c_void,
}

impl<T> Drop for MappedRegisteredResource<'_, T> {
    fn drop(&mut self) {
        unsafe { (ENCODE_API.unmap_input_resource)(self.resource.encoder.ptr, self.map_ptr) }
            .result(self.resource.encoder)
            .expect("The encoder pointer and mapped handle should be valid.");
        self.resource.mapped.set(false);
    }
}

impl<T> EncoderInput for MappedRegisteredResource<'_, T> {
    fn pitch(&self) -> u32 {
        self.resource.pitch
    }
